...
public class ImmDSForecastProvider {
        ...
        private static final String SYSTEM_EVALUATION_DOSE_STATUS = "http://terminology.hl7.org/CodeSystem/immunization-evaluation-dose-status";

        private static final Coding EVALUATION_DOSE_STATUS_VALID = new Coding().setCode("valid").setDisplay("Valid")
                        .setSystem(SYSTEM_EVALUATION_DOSE_STATUS);
        ...
        private static final Coding DISEASE_DIPHTHERIA = new Coding().setCode("397428000").setDisplay("Diphtheria")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_PERTUSSIS = new Coding().setCode("27836007").setDisplay("Pertussis")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_TETANUS = new Coding().setCode("76902006").setDisplay("Tetanus")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_INFECTION_CAUSED_BY_HUMAN_POLIOVIRUS = new Coding().setCode("721764008")
                        .setDisplay("Infection caused by Human poliovirus")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_MEASLES = new Coding().setCode("14189004").setDisplay("Measles")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_MUMPS = new Coding().setCode("36989005").setDisplay("Mumps")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_RUBELLA = new Coding().setCode("36653000").setDisplay("Rubella")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_PNEUMOCOCCAL_INFECTIOUS_DISEASE = new Coding().setCode("16814004")
                        .setDisplay("Pneumococcal infectious disease")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_MENINGOCOCCAL_INFECTIOUS_DISEASE = new Coding().setCode("23511006")
                        .setDisplay("Meningococcal infectious disease")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_HAEMOPHILUS_INFLUENZAE_TYPE_B_INFECTION = new Coding().setCode("709410003")
                        .setDisplay("Haemophilus influenzae type b infection")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_VIRAL_GASTROENTERITIS_DUE_TO_ROTAVIRUS = new Coding().setCode("415822001")
                        .setDisplay("Viral gastroenteritis due to Rotavirus")
                        .setSystem(SYSTEM_SNOMED_CT);
        ...
        private static final Coding DISEASE_INFLUENZA_CAUSED_BY_SEASONAL_INFLUENZA_VIRUS = new Coding()
                        .setCode("719590007")
                        .setDisplay("Influenza caused by seasonal influenza virus")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_VARICELLA = new Coding().setCode("38907003").setDisplay("Varicella")
                        .setSystem(SYSTEM_SNOMED_CT);
        private static final Coding DISEASE_HERPES_ZOSTER = new Coding().setCode("4740000").setDisplay("Herpes zoster")
                        .setSystem(SYSTEM_SNOMED_CT);
        ...
        public Parameters ImmDSForecastOperation(
                ...
                try {
                        ...
                        try {
                                ...
                                forecastHandlerCore.forecast(doseList, patientRecord, imms,
                        ...
                        for (VaccinationDoseDataBean dose : doseList) {
                                ...
                                Immunization immunization = immunizationList
                                                .get(Integer.parseInt(dose.getVaccinationId()) - 1);
                                if (snomedCodings != null && immunization != null) {
                                        for (Coding snomedCoding : snomedCodings) {
                                                ...
                                                immunizationEvaluation
                                                                .setStatus(ImmunizationEvaluationStatus.COMPLETED);
                                                ...
                                                immunizationEvaluation.getTargetDisease().addCoding()
                                                                .setSystem(SYSTEM_LONESTAR)
                                                                .setCode(dose.getForecastCode())
                                                                .setDisplay(dose.getForecastCode());
                                                immunizationEvaluation
                                                                .setImmunizationEvent(new Reference(immunization));
                                                if (dose.getStatusCode() == VaccinationDoseDataBean.STATUS_INVALID) {
                                                        immunizationEvaluation.getDoseStatus()
                                                                        .addCoding(EVALUATION_DOSE_STATUS_VALID);
                                                } else {
                                                        immunizationEvaluation.getDoseStatus()
                                                                        .addCoding(EVALUATION_DOSE_STATUS_NOTVALID);
                                                }
                                                if (dose.getReason() != null && !dose.getReason().equals("")) {
                                                        immunizationEvaluation.addDoseStatusReason()
                                                                        .setText(dose.getReason());
                                                }
                                                try {
                                                        int theValue = Integer.parseInt(dose.getDoseCode());
                                                        immunizationEvaluation.setDoseNumber(
                                                                        new PositiveIntType().setValue(theValue));
                                                } catch (NumberFormatException nfe) {
                                                        // ignore
                                                }
                                                parameters.addParameter().setName("evaluation")
                                                                .setResource(immunizationEvaluation);
                                        }
                                }
                        }
                        ...
                        for (ImmunizationForecastDataBean forecast : resultList) {
                                ImmunizationRecommendationRecommendationComponent comp = new ImmunizationRecommendationRecommendationComponent();
                                ...
                                comp.addVaccineCode().addCoding().setCode(doseDueCode)
                                                .setDisplay(forecast.getForecastName())
        ...
        private void addDateCriterion(ImmunizationRecommendationRecommendationComponent comp,
                ...
                if (date != null) {
                        ImmunizationRecommendationRecommendationDateCriterionComponent dateCriterion = comp
                                        .addDateCriterion();
                ...
                for (Immunization immunization : immunizationList) {
                        ...
                        String vaccineCvx;
                        {
                                ...
                                if (immunization.getVaccineCode() == null
                                                || (coding = immunization.getVaccineCode()
                                                                .getCodingFirstRep()) == null) {
                                        throw new UnprocessableEntityException(
                                                        "No vaccine code for vaccination #" + vaccCount);
                                }
                                if (coding.getSystem() != null
                                                && !coding.getSystem().equals("http://hl7.org/fhir/sid/cvx")
                                                && !coding.getSystem().equals("")) {
                                        throw new UnprocessableEntityException(
                                                        "Unable to read code, unspported code system '"
                                                                        + coding.getSystem() + "' at vaccination #"
                                                                        + vaccCount);
                                }
                        ...
                        int vaccineId = 0;
                        {
                                ...
                                if (cvxCode == null) {
                                        throw new UnprocessableEntityException(
                                                        "Code '" + vaccineCvx + "' is not recognized for vaccination #"
                                                                        + vaccCount);
                                } else {
                        ...
                        org.immregistries.lonestar.core.model.Immunization imm = new org.immregistries.lonestar.core.model.Immunization();
